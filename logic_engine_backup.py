...
class LogicEngine:
    ...
    def _parse_order_text(self, frame_crop: np.ndarray, dt_boxes: list, rec_res: list, is_flip: bool) -> List:
        ...
        # 1. 找出 takeout_y / takeout_x / nn_x，同時收集純數字框 (皆使用校正後座標)
        takeout_y = None
        takeout_x = None
        digit_boxes = []  # (corrected_cy, digit_value)
        ...
        for dt_box, (text, score) in zip(flip_dt, flip_res):
            ...
            cy = np.mean([pt[1] for pt in corrected_box])
            x_coords = [pt[0] for pt in corrected_box]
            ...
            if take_out_match and take_out_match[1] > 60:
                ...
                # 注意：這裡原本有 break，若確定後面沒有更精準的資訊再保留

        # 2. 過濾有效文字區塊 (使用校正後座標進行空間過濾)
        valid_items = []
        for dt_box, (text, score) in zip(flip_dt, flip_res):
            ...
            corrected_box = skew.correct_box(dt_box)
            cy = np.mean([pt[1] for pt in corrected_box])
            cx = np.mean([pt[0] for pt in corrected_box])
            h = abs(min(corrected_box[3][1], corrected_box[2][1]) - max(corrected_box[0][1], corrected_box[1][1]))
            ...
            valid_items.append({
                'text': text,
                'score': score,
                'dt_box': dt_box,  # 保留原始框供參考
                'cx': cx,          # 校正後 cx
                'cy': cy,          # 校正後 cy
                'h': h             # 校正後高度
            })

        # 3. 將同個高度的文字合併成同一列 (校正後座標讓行合併極度精準)
        rows = []
                ...
                if match_score > 40:
                    ...
                    if leading_digit is not None and matched_str not in self.menus_with_digits:
                        ...
                        if match_num == 1:
                            ...
                            # 關鍵優化：方法三 (rec_pred) 的交集 Crop 區域計算
                            if nn_x is not None:
                                # 1. 在擺正的邏輯座標系中，交點就是 (數量欄的X , 當前菜單行的Y)
                                logical_cx = nn_x
                                logical_cy = menu_item['cy']

                                # 2. 將這個邏輯交點，反向旋轉回帶有傾斜的原始真實影像座標
                                ...
                                alone_res, _ = self.rec_pred.predict_quantity(
                                    ...
                                    cx=real_cx,
                                    cy=real_cy,
                                    h=menu_item['h'],  # 高度與寬度在小角度傾斜下直接使用即可
                                    w=nn_w,
                                    is_flip=is_flip
                                )
